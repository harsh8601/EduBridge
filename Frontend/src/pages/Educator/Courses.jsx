                ...
                <tbody>
                    ...
                    <tr key={index} className='border-b hover:bg-gray-50 transition duration-200'>
                        <td className='py-3 px-4 flex items-center gap-4'>
                            {course?.thumbnail ?<img src={course?.thumbnail} className='w-25 h-15 object-cover rounded-md' alt="" />:<img src={img} className='w-25 h-15 object-cover rounded-md' alt="" />}
                            <span>{course?.title}</span>
                        </td>
                        {course?.price ?<td className='px-4 py-3'>₹{course?.price}</td> :
                        <td className='px-4 py-3'>₹ NA</td>}

                        <td className='px-4 py-3'><span className={`px-3 py-1 rounded-full ${course.isPublished ?"bg-green-100 text-green-600" :" bg-red-100 text-red-600"} text-xs`}>{course.isPublished ? "Published":"Draft"}</span></td>
            ...
            {creatorCourseData?.map((course,index)=>(
             <div key={index} className='bg-white rounded-lg shadow p-4 flex flex-col gap-3'>
                <div className='flex gap-4 items-center'>
                    {course?.thumbnail ?<img src={course?.thumbnail} className='h-16 w-16 rounded-md object-cover' alt="" />:<img src={img} className='h-16 w-16 rounded-md object-cover' alt="" />}
